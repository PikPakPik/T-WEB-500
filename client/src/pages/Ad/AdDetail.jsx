    ...
    (async () => {
      try {
        ...
        if (window.localStorage.token) {
          ...
          const res = await fetch("http://localhost:3001/appliedAdvert", {
            method: "GET",
            headers: header,
          });
          const appliedAdvert = await res.json();

          if (fetchedAd.jobInformation.length > 0)
            setIsSaved(fetchedAd.jobInformation[0].isSaved);

          if (appliedAdvert.length > 0) {
            const filteredData = appliedAdvert.filter((item) => {
              return item.advertissementId === parseInt(avertissementId);
            });
